"""
Tests for CollisionDetector — collision-detection.md R1–R4 (all scenarios)
...
class TestR1DetectCollision:
    ...
    def test_r1_s1_two_cars_at_same_cell_produces_record(self):
        """R1.S1 — Two cars at (2,2) → one collision record at step 1."""
        a = make_car("A", 2, 2, "E")
        b = make_car("B", 2, 2, "W")
        records = CollisionDetector.check([a, b], step=1)
        assert records == ["A B collides at (2,2) at step 1"]
...
class TestR2CollidersStopped:
    ...
    def test_r2_colliding_cars_are_marked_stopped(self):
        ...
        a = make_car("A", 2, 2)
        b = make_car("B", 2, 2)
        CollisionDetector.check([a, b], step=1)
    ...
    def test_r2_non_colliding_car_is_not_stopped(self):
        ...
        b = make_car("B", 2, 2)
        c = make_car("C", 9, 9)
        CollisionDetector.check([a, b, c], step=1)
    ...
    def test_r2_s1_stopped_cars_excluded_from_subsequent_checks(self):
        ...
        records_step2 = CollisionDetector.check([a, b], step=2)
...
class TestR3RecordFormat:
    ...
    def test_r3_s1_two_car_record_format(self):
        """R3.S1 — Exact string: 'A B collides at (2,2) at step 1'."""
        a = make_car("A", 2, 2)
        b = make_car("B", 2, 2)
        records = CollisionDetector.check([a, b], step=1)
        assert records == ["A B collides at (2,2) at step 1"]
    ...
    def test_r3_s2_three_car_record_names_alphabetical(self):
        """R3.S2 — Three cars: names are alphabetically sorted."""
        # Cars named B, A, C in declaration order → record must say 'A B C'
        b = make_car("B", 5, 5)
        a = make_car("A", 5, 5)
        c = make_car("C", 5, 5)
        records = CollisionDetector.check([b, a, c], step=3)
        assert records == ["A B C collides at (5,5) at step 3"]

    def test_r3_reversed_name_order_still_alphabetical(self):
        """Extra — names Z and A collide; record must be 'A Z ...' not 'Z A ...'."""
        z = make_car("Z", 1, 1)
        a = make_car("A", 1, 1)
        records = CollisionDetector.check([z, a], step=2)
        assert records == ["A Z collides at (1,1) at step 2"]

    def test_r3_step_number_in_record(self):
        """Extra — step number is correctly embedded in the record."""
        a = make_car("A", 3, 4)
        b = make_car("B", 3, 4)
        records = CollisionDetector.check([a, b], step=7)
        assert records == ["A B collides at (3,4) at step 7"]
...
class TestR4Step0PlacementConflict:
    ...
    def test_r4_s1_two_cars_same_initial_position(self):
        ...
        a = make_car("A", 3, 3, "N")
        b = make_car("B", 3, 3, "S")
        records = CollisionDetector.check([a, b], step=0)
        assert records == ["A B collides at (3,3) at step 0"]
        assert a.stopped is True
        assert b.stopped is True
...
class TestMultipleSimultaneousCollisions:
    ...
    def test_two_collision_groups_ordered_by_x_then_y(self):
        ...
        c = make_car("C", 2, 2)
        d = make_car("D", 2, 2)
        records = CollisionDetector.check([a, b, c, d], step=1)
        assert len(records) == 2
        assert records[0] == "C D collides at (2,2) at step 1"
        assert records[1] == "A B collides at (7,7) at step 1"
    ...
    def test_two_collision_groups_same_x_ordered_by_y(self):
        ...
        c = make_car("C", 3, 2)
        d = make_car("D", 3, 2)
        records = CollisionDetector.check([a, b, c, d], step=2)
        assert records[0] == "C D collides at (3,2) at step 2"
        assert records[1] == "A B collides at (3,8) at step 2"
    ...
    def test_only_colliding_groups_stopped_in_multi_collision(self):
        ...
        c = make_car("C", 7, 7)
        d = make_car("D", 7, 7)
        CollisionDetector.check([a, b, c, d], step=1)
        assert a.stopped and b.stopped
